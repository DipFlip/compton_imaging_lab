...
const en=[.15,.2,.3,.4,.5,.6,.8,1],la=[.8239,.4239,.1961,.1301,.1015,.0857,.06843,.05876],br=[.2899,.1838,.1186,.09563,.08328,.07515,.06443,.05728];
export const dot=(a,b)=>a.reduce((s,x,i)=>s+x*b[i],0),sub=(a,b)=>a.map((x,i)=>x-b[i]),norm=a=>Math.hypot(...a),unit=a=>a.map(x=>x/norm(a));
export function total(e){let i=0;while(i<en.length-2&&e>en[i+1])i++;let t=Math.log(Math.max(.15,e)/en[i])/Math.log(en[i+1]/en[i]);let mix=j=>(138.905*la[j]+239.712*br[j])/378.617;return Math.exp(Math.log(mix(i))*(1-t)+Math.log(mix(i+1))*t);}
...
export function response(s,a,b,yawA=a.yaw??0,yawB=b.yaw??0){let incoming=unit(sub(a,s)),out=unit(sub(b,a)),c=Math.max(-1,Math.min(1,dot(incoming,out))),e=1/(1+(1-c)/.511),ai=area(rotateY(incoming,-yawA)),ao=area(rotateY(out,-yawB)),escapeArea=area(rotateY(out,-yawA)),mu=total(1)*rho,mp=total(e)*rho;
let w=ai/(4*Math.PI*norm(sub(a,s))**2)*(1-Math.exp(-mu*volume/ai))*compton(1)/total(1)*Math.exp(-mp*volume/escapeArea/2)*kn(c)*ao/norm(sub(b,a))**2*(1-Math.exp(-mp*volume/ao))*Math.max(0,1-compton(e)/total(e));return {e:1-e,w};}
// First-order propagation of independent uniform unknown coordinates in both crystals.
export function spread(s,a,b,res){
  ...
  for(let k=0;k<6;k++){
    ...
    const plus=response(s,aa,bb,a.yaw,b.yaw).e;
    for(let axis=0;axis<3;axis++)target[axis]-=2*delta[axis];
    const minus=response(s,aa,bb,a.yaw,b.yaw).e;
    variance+=((plus-minus)/(2*h))**2*dims[k%3]**2/12;
  }
  return Math.sqrt(variance+(res/100/2.355)**2*.662*response(s,a,b).e);
}
export const NX=72,NY=36,NB=100,M=16*NB;
// The known 1 MeV sum constrains two independent scintillation measurements.
export const energySigma=(e,res)=>res/100/2.355*Math.sqrt(.662*Math.max(0,e*(1-e)));
...
export function addSpectrum(out,base,mean,sigma,weight,thresholdKeV=20){
  const lower=thresholdKeV/1000,upper=1-lower;
  if(sigma<1e-10){const k=Math.floor(mean/.01);if(mean>=lower&&mean<upper&&k>=0&&k<NB)out[base+k]+=weight;return;}
  const lo=Math.max(0,Math.floor(Math.max(lower,mean-7*sigma)/.01));
  const hi=Math.min(NB,Math.ceil(Math.min(upper,mean+7*sigma)/.01));
  for(let k=lo;k<hi;k++){
    const left=Math.max(lower,k*.01),right=Math.min(upper,(k+1)*.01);
...
export function orderedSpectrum(s,det,p,i,j,order){
  const a=det[order?j:i],b=det[order?i:j],r=response(s,a,b),mean=order?1-r.e:r.e;
  const sigma=Math.sqrt(spread(s,a,b,0)**2+energySigma(mean,p.res)**2),out=new Float64Array(NB);
  addSpectrum(out,0,mean,sigma,r.w,p.thresholdKeV??20);return out;
}
