...
const $=id=>document.getElementById(id);let p={sep:40,gapA:0,gapB:0,radius:100,az:35,el:15,res:2.6,iterations:30,thresholdKeV:20,rotationA:0,rotationB:0,activityUCi:1000,measurementSeconds:3600,generator:'volume',seed:42},worker,result,records=[],busy=true,timer,roomView;
...
function sync(){for(let key of ['sep','gapA','gapB','rotationA','rotationB','thresholdKeV','az','el','radius','res','iterations']){$(key).value=p[key];$(key+'Value').textContent=p[key]+' '+$(key).dataset.unit;}drawScene();}
...
const sweepKey=q=>JSON.stringify([q.gapA,q.gapB,q.radius,q.az,q.el,q.res,q.rotationA,q.rotationB,q.thresholdKeV]);
function checkSweep(){
...
$('exportSweep').onclick=()=>{
  ...
  const keys=['sep','gapA','gapB','rotationA','rotationB','thresholdKeV','radius','az','el','res','eff','effSE','armRms','positionArmRms','armBias','armValidFraction','reverseFraction','photonsFor500','activityUCi','measurementSeconds','expectedEvents'];
